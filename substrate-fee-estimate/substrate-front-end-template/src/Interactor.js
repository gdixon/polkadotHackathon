...
import { Grid, Form, Button, Dropdown, Input, Label } from 'semantic-ui-react';
...
function Main(props) {
    ...
    // check if the estimate method should be disabled or not
    const isDisabled = function() {
        // check if the transaction is in vaild state
        if (!palletRpc || !callable) return true;
        // map the params where a value is present
        const params = inputParams.filter((value) => value.value);
        // discover the number of params required
        const reqParamLen = api.tx[palletRpc][callable].meta.args.length;
        // not a valid transaction when parts are missing
        if (params.length != reqParamLen) return true;
    };

    // retrieve the estimated tx costs from sidecar
    const getEstimate = async function () {
        // only update estimate if we're not disabled
        if (!isDisabled()) {
            // construct the given transaction
            const transaction = await api.tx[palletRpc][callable](...inputParams);
            // post the transaction details to the sidecar
            const estimate = await axios.post("/transaction/fee-estimate", { 
                tx: transaction.toHex() 
            });
            // set the estimate fee by reading "partialFee" from the sidecar response
            if (estimate.data.partialFee) setEstimate(estimate.data.partialFee);
        }
    };

    return (
        <Grid.Column width={8}>
            <h1>Pallet Interactor with fee estimate</h1>
            <Form>
                ...
                </Form.Group>
                {
                    (estimate ?
                        <Label basic color="teal">
                            This action is estimated to cost ~{Number(estimate / 10000000).toFixed(4)} mDOT -> sign the tx only if you accept the fee.
                        </Label> :
                        <div></div>
                    )
                }
