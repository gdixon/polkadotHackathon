...
import CustContract, {defaultGasLimit, displayCust} from "./CustContract";
...
export default function Main(props) {
    ...
    const updateBalanceOf = async (accountAddress) => {
        const result = await custContract.query.balanceOf(accountAddress, 0, defaultGasLimit, accountAddress);
                ...
                <Table.Body>{accounts.map(account =>
                    <Table.Row key={account.address}>
                        ...
                        <Table.Cell width={3}>{
                            ...
                            displayCust(balances[account.address])
                        }</Table.Cell>
                    </Table.Row>
                )}
                </Table.Body>
